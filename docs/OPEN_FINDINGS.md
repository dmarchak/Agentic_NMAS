...
| C15 | **A write whose commit failed is stranded for ever, and the run that strands it reports success.** Measured 2026-09-25 on `labs/r6`: the 20:17 run wrote `configs/r6.cfg` (+16/-1, header `from Oxidized HEAD dc871ef`) and its commit failed (no identity, before the fix). The 20:30 run found every destination matching and **exited 0 at "Nothing to do - the clab VM already matches Oxidized." before the commit step**. The file stays staged and unversioned, no later run will see a difference, and job health reads the job as ok. **Not what the operator first read**: `destinations()` returns every mapped destination and the commit step would commit it; the early exit is the defect. Wanted: reconcile every managed destination every run, before the early exit and before any copy. A dirty tracked file is committed only when it is **provably this job's output** (byte-identical to the sanitiser's output for the Oxidized ref named in its own header). Anything else is **refused for that destination, neither committed nor overwritten**, and named, and the run exits non-zero so job health shows it. | build | NSOT_PLAN.md P.1 (r5 retirement, step 2) |
...
(A2–D2, with A4, B4, C6–C10 and C12–C15 added the same day; D3 and C11 added and fixed), 4 in the plan without a stage (E1–E4, one of which is Stage 3.3's
