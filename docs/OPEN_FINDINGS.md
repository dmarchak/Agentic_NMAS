...
| C11 | **Deleting a device leaves it half-managed in five places, and there is no "retire" operation.** Measured 2026-09-25 on r5. `POST /device/<ip>/delete` removes the CSV row (with the **only copy of r5's rotated credential**: no override, shared with no other device), closes connections and clears topology caches. It also **derives the active list**, a write inferring its list. It leaves: **(1)** the manifest entry, so r5 stays **bound to `cisco_iosxe/base.j2`** and every future approval validates against a device NMAS no longer manages; **(2)** the clab sync map, which is built from the manifest while `platform_of()` reads the inventory, so r5 would be reported on every sync as INCOMPLETE with a **false reason** ("lab names no platform") and its `r5.cfg` would become unmapped; **(3)** the heartbeat rules, whose population is NetBox, so r5 keeps a rule and **alerts for ever**; **(4)** NetBox device 9 and its objects, not tagged `nmas-managed` and not in the created record, so provenance-based Remove correctly cannot touch it (A3); **(5)** `host_vars/r5.yml` and `golden/r5.cfg` in the working tree. (5) is history and should stay in git, but out of the tree. Wanted: a retire operation, one commit that removes the files (history kept), releases the identity, and names what it cannot do (NetBox, Oxidized). | build | NSOT_PLAN.md P.1 (r5) |
...
(A2–D2, with A4, B4, C6–C12 added the same day; D3 added and fixed), 4 in the plan without a stage (E1–E4, one of which is Stage 3.3's
...
By kind: **15 build**, **3 decide-then-build**, **2 decide**, **2 verify**.
