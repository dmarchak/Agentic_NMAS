...
| B5 | **`data/key.key` has no copy anywhere measured, and every encrypted secret depends on it.** Losing it makes the credential store, `devices.csv`, and every encrypted setting unrecoverable. The break-glass record holds device credentials in plaintext, but not the key, the credential profiles or the template secrets. P.2 backs up NetBox, not `data/`. Whether a vzdump job images the VM is unanswered (`/etc/pve/jobs.cfg`). Until one of those is known, this is a single copy. | decide, then build | NSOT_PLAN.md P.2 (coverage) |
...
**24 open** (counted from the rows, 2026-09-25): 20 recorded only in prose
(A2–D2, with A4, B5, C6–C10, C12–C14 and C17 added the same day; B4, D3, C11, C15, C16, C18 and C19 added and closed), 4 in the plan without a stage (E1–E4, one of which is Stage 3.3's
...
By kind: **15 build**, **4 decide-then-build**, **4 decide**, **2 verify**.
