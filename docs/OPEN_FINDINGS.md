...
| C16 | **The per-platform heartbeat window is the wrong shape: clock rate is a property of each DEVICE, and it moves.** Measured 2026-09-25 (operator's interval run, then the device-vs-arrival discriminator): every device's own clock puts its heartbeats 300.0 s apart (±0.5 s), so EEM is exact and delivery is not the cause (r2 arrives at 300.0). But the vIOS clocks run at **0.94 (s1), 0.94 (s2), 0.75 (s4) and 0.55–0.61 (s3, varying)** of real time. Against the 999 s vIOS window: s1 and s2 **FAIL too wide** (two misses ≈ 954 s never alert); s3 **FAILS too narrow** (2 × 546 = 1092 s, so one miss could false-alarm); s4 passes. "vIOS runs at 75%" was s4 generalised to a platform, the same shape as the 60 s slack it replaced. s3's variable rate fits an emulated clock losing ticks under load (it is the busiest switch and the manager's gateway): a hypothesis, not measured. **The windows must be per device, from that device's own measured arrivals, and refreshed**, since a rate that moves makes any stored window a claim with a date. Design in NSOT_PLAN P.1 (step 5). | build | NSOT_PLAN.md P.1 (step 6 acceptance measurement) |
...
(A2–D2, with A4, B4, C6–C10, C12–C14 and C16 added the same day; D3, C11 and C15 added and fixed), 4 in the plan without a stage (E1–E4, one of which is Stage 3.3's
...
By kind: **16 build**, **3 decide-then-build**, **3 decide**, **2 verify**.
