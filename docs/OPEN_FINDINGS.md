...
| A4 | **NetBox records every device's platform as `ios`, the C8000v routers included.** Measured 2026-09-25: r1–r6 (C8000v, IOS-XE) and s1–s4 (vIOS-L2) are all `ios`, a slug `platform.py` does not even map. The NSoT manifest has it right (r1–r6 `cisco_iosxe`, s1–s4 `cisco_ios`). A NetBox-sourced list would therefore fail to resolve any device's platform, and anything keyed on NetBox's platform gives routers switch behaviour. It was found because the heartbeat window has to be per platform. That generator reads the dialect from the manifest and says why. The import's platform source is the defect. | build | NSOT_PLAN.md P.1 (step 4, r2) |
...
**19 open** (counted from the rows, 2026-09-25): 15 recorded only in prose
(A2–D2, with A4, B4, C6, C7, C8 and C9 added the same day; D3 added and fixed), 4 in the plan without a stage (E1–E4, one of which is Stage 3.3's
...
By kind: **13 build**, **3 decide-then-build**, **1 decide**, **2 verify**.
