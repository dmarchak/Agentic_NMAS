...
| **Jenkins tab** | **fold in** | CI is real and the pipeline's stage 3 gate depends on it. But it is *part of a deploy*, not a destination: it belongs as a CI strip on the deploy record in Fleet → Changes. `modules/pipeline.py` and `pipeline_builder.py` stay — they are tested and Phase 3 depends on them. |
